function[A, b] = boarding_cons(N_b, N_s, N_x, N_odPair, N_odPair2, H_p, M, hat_B, AllowedLoadTime)
...
A = zeros(2*(N_s-1)*(N_s-1)*H_p*N_b+2*N_b*H_p+2*(N_s-1)*N_b*H_p+ (N_s-1)*H_p, N_x);
b = zeros(2*(N_s-1)*(N_s-1)*H_p*N_b+2*N_b*H_p+2*(N_s-1)*N_b*H_p+ (N_s-1)*H_p, 1);
...
startnew4 = startnew3+ (N_s-1)*(N_s-1)*H_p*N_b;
